<template>
  <div class="example">
    <Renderer ref="renderer" :antialias="true">
      <OrthographicCamera :position="[25, 10, 10]" :up="[0, 0, 1]">
        <OrbitControls :target="[25, 0, 0]" />
      </OrthographicCamera>
      <Scene background="#f9f9f9">
        <Mesh v-for="i in 6" :key="i" :rotation="rot" :position="[i * 7.5, 0, 0]">
          <MeshBasicMaterial color="#aaa" />
          <BoxGeometry :name="`cube${i}`" :width="5" :height="5" :depth="2" />
        </Mesh>
        <LineSegments v-for="i in 6" :key="i" :rotation="rot" :position="[i * 7.5, 0, 0]">
